...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        print(self.evenOddPartition([5, 8, 9, 2, 0]))
        print(self.evenOddPartition([1, 0, 1, 0, 1, 0]))
        print(self.evenOddPartition([1, 3, 5, 7, 9]))
        print(self.evenOddPartition([]))
    }

    
    func evenOddPartition(_ arr: [Int]) -> [[Int]] {
        var arrayResultEven: [Int] = []
        var arrayResultOdd: [Int] = []
        var arrayResult: [[Int]] = [[]]
        arrayResult.removeAll()
        for n in arr{
            if n % 2 == 0 {
                arrayResultEven.append(n)
            }else{
                arrayResultOdd.append(n)
            }
        }
        arrayResult.append(arrayResultEven)
        arrayResult.append(arrayResultOdd)
        return arrayResult
